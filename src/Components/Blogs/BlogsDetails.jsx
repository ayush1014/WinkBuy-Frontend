...
export default function BlogsDetails() {
    ...
    const fetchBlogDetails = useCallback(async (attempt = 0) => {
        try {
            ...
            setBlogs(response.data); // Set the array of blogs returned from the API
            setIsLoading(false);
        } catch (error) {
            if (attempt < maxRetries) {
                setTimeout(() => fetchBlogDetails(attempt + 1), retryDelay); // Retry after a delay
            } else {
                console.error('Error fetching blog details after multiple attempts:', error);
                setIsLoading(false); // Stop loading after max retries
            }
        }
    }, [blogname, maxRetries, retryDelay]);

    useEffect(() => {
        fetchBlogDetails(); // Initial fetch attempt
    }, [fetchBlogDetails]);

...
export default function TypeCategories() {
  ...
  const fetchCategories = useCallback(async (attempt = 0) => {
    try {
      setIsLoading(true);
      const response = await api.get(`/main-category/${type}/categories`);
      ...
      if (attempt < maxRetries) {
        setTimeout(() => fetchCategories(attempt + 1), retryDelay); // Retry after a delay
      } else {
  ...
  useEffect(() => {
    fetchCategories(); // Initial fetch attempt
  }, [fetchCategories]);
